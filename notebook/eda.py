...
def create_agg_features(
    ...
    train_m[features].fillna(train_m[features].mean(), inplace=True)
    test_m[features].fillna(train_m[features].mean(), inplace=True)
...
print(f"Before Train Features: {train.shape}")
print(f"Before Test Features: {test.shape}")
train, test = create_agg_features(train, test, data_dir)
